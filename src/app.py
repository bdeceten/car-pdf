...
class App():
    def __init__(self):
        ...
        self.pdfEntry = Entry(self.root, width=50)
        ...
        self.button = Button(self.root,width=10, command=self.CreatePDF, text="Remplir")
    ...
    def DropPdf(self,event):
        self.pdfEntry.delete(0,END)
        self.pdfEntry.insert(0,event.data)
    ...
    def CreatePDF(self,event=None):
        ...
        km=self.kmEntry.get()
        pdf=self.pdfEntry.get()

        if nom == "" or prenom == "" or date == "" or km == "" or pdf == "":
            self.canvas.itemconfigure(self.errorText, text = "Erreur : champ vide", fill="red")
            return 0

        if not re.match(r"[0-9]{2}\/[0-9]{2}\/[0-9]{4}", date):
            self.canvas.itemconfigure(self.errorText, text = "Erreur : format de la date", fill="red")
            return 0

        if not re.match(r"[1-9][0-9]*", km):
            self.canvas.itemconfigure(self.errorText, text = "Erreur : format des km", fill="red")
            return 0
        try:
            ...
            packet = io.BytesIO()
            can = cv.Canvas(packet, pagesize=(xmax,ymax))
            can.drawImage("ressources/signature.png", xmax*0.072, ymax*0.193, height=ymax*0.084, width=ymax*0.084*808/263)
            can.save()
            packet.seek(0)
            packetReader = PdfFileReader(packet)
            ...
            #Output PDF
            pdfOutput = open(f"output/{date[3:5]}_{date[0:2]}_{nom.upper()}.pdf", 'wb')
            pdfWriter.write(pdfOutput)
            pdfOutput.close() 
            self.canvas.itemconfigure(self.errorText, text = "Pdf généré", fill="green")
            
        except Exception:
            self.canvas.itemconfigure(self.errorText, text = "Erreur : chemin d'accès au pdf", fill="red")
            return 0
